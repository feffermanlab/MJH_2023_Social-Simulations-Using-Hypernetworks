...
seedVect <- round(rnorm((numberSims * length(selectiveSet)), mean = 500000, sd = 100000))
...
for(s in 1:numberSims) {
  ...
  for(m in 1:length(selectiveSet)) {
    ...
    popData <- create_population_data(N, initialAges = sample(ageDist, size = N, replace = TRUE), ageBias = 0, selectGradient = selectGradient)
